document.addEventListener("DOMContentLoaded", async function () {
    ...
    if (page === "index.html" || page === "") {
        ...
        if (!isNaN(characterIndex) && characterIndex >= 0 && characterIndex < characters.length) {
            ...
            document.getElementById(`character-name`).innerHTML = `${getFullName(character,'personal')}<br>&nbsp&nbsp<i><sup><sub>${[character.name[0]?.[2],character.name[1]?.[2],character.name[2]?.[2]].filter(Boolean).join(' ')}</sub></sup></i><hr>`;
