document.addEventListener("DOMContentLoaded", async function () {
    function consoleFormat(input) {
        return input.replace(/<\//g,'<').replace(/<h2>/g,'<<NEWLINE>>').replace(/<p>/g,'\n').replace(/\n\n/g,'\n').replace(/<<NEWLINE>>/g,'\n')
    }
    ...
    if (page === "index.html" || page === "") {
        ...
        characters.forEach((character, index) => {
            ...
            li.innerHTML = `<a href="character.html?index=${index}" class="block text-lg text-gray-800">${getFullName(character, 'official')
                }${character.name[0][2] ? affix([character.name[0]?.[2], character.name[1]?.[2], character.name[2]?.[2]].filter(Boolean).join(' '), '<br>') : ''
                }</a > `;
            list.appendChild(li);
        });
        ...
        if (!isNaN(characterIndex) && characterIndex >= 0 && characterIndex < characters.length) {
            ...
            document.getElementById(`character-name`).innerHTML = `${getFullName(character, 'personal')}${character.name[0][2] ? affix([character.name[0]?.[2], character.name[1]?.[2], character.name[2]?.[2]].filter(Boolean).join(' '),'<br>&nbsp&nbsp<i><sup><sub>','</sub></sup></i><hr>') : ''}`;
